...
int main(int argc, char** argv) {
    ...
    for(size_t i = 0; i < strlen(argv[2]); ++i) {
        if(isalpha(argv[2][i])) erro("<Intervalo de envios> deve ser unsigned int.");
    }
    ...
    for(int i = 0; i < strlen(argv[4]); ++i) {
        if(isalpha(argv[4][i])) erro("<min> deve ser unsigned int.");
    }
    for(int i = 0; i < strlen(argv[5]); ++i) {
        if(isalpha(argv[5][i])) erro("<max> deve ser unsigned int.");
    }
